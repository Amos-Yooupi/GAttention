...
class GraphAttentionLayer(nn.Module):
    def __init__(self, in_dim, out_dim, n_heads, att_mode="linear", activation="relu"):
        ...
        self.fc = nn.Sequential(activation, nn.Linear(self.in_dim, self.out_dim))
        if att_mode == "linear":
...
class FusionGraphAttentionLayer(nn.Module):
    def __init__(self, in_features, out_features, n_heads, att_mode="linear", activation="relu"):
        ...
        if activation == "relu":
            ...
            raise ValueError("Invalid activation function")
        self.fc = nn.Sequential(activation, nn.Linear(in_features, out_features))
    ...
    def forward(self, x, adj):
        ...
        fusion_out = self.fc(graph_message_x) + att_out
        return fusion_out, att_prob
