...
import torch
from GraphAttentionNetwork import GraphAttentionLayer, FusionGraphAttentionLayer, GraphNeuralNetwork
from Distill_Block import DistillBlock
from SharedSpareMoe import SharedSpareTimeMOE, BasicExpert, BasicTimeExpert
...
class RGNNBlock(nn.Module):
    def __init__(self, in_dim, out_dim, input_length, num_heads_for_time,
        ...
        self.time_spare_moe = SharedSpareTimeMOE(in_dim, in_dim, num_expert,
                                                 ...
                                                 input_length, top_k)
        self.share_expert = BasicTimeExpert(in_dim, in_dim, in_dim, 3, input_length, input_length)

        self.fc = BasicExpert(in_dim, in_dim, in_dim, 3)
        # 残差 + 层归一化
        ...
        # else:
        #     self.gat_layers = GraphNeuralNetwork(in_dim, activation=activation)
    ...
    def forward(self, x, *args):
        ...
        """
        B, L, D = x.shape
        # multi_head_att_x, self.time_att_weight = self.time_multi_head_att(x, x, x)
        # self.time_att_weight = self.time_att_weight.detach().cpu()
        # # 残差 + 层归一化
        # multi_head_att_x = self.add_norm_1(x, multi_head_att_x)
        # Time-aware spare moe --- 时序专家模块
        time_aware_moe_x = self.time_spare_moe(x)
        # time_aware_moe_x = self.fc(multi_head_att_x)
        # time_aware_moe_x = self.share_expert(multi_head_att_x)
        # 残差 + 层归一化
        time_aware_moe_x = self.add_norm_2(x, time_aware_moe_x)  # [B*N, L, D]
        # DistillBlock - 蒸馏模块
        # distill_x = self.distill_block(time_aware_moe_x).contiguous().view(B, N, L // 2, D)
        # GAT 层 -- 拼接time维度
        # time_concat_x = time_aware_moe_x.contiguous().view(B, N, L, D).permute(0, 2, 1, 3).contiguous().view(-1, N, D)
        # node_att_x, self.node_att_weight = self.gat_layers(time_concat_x, adj.repeat(L, 1, 1))
        # 复原所有维度
        # node_att_x = node_att_x.contiguous().view(B, L, N, self.out_dim).permute(0, 2, 1, 3)
        return time_aware_moe_x
...
class RGNN(nn.Module):
    def __init__(self, in_dim, hidden_dim, input_length, num_heads_for_time,
        ...
        for i in range(num_blocks):
            ...
            self.blocks.append(RGNNBlock(in_dim, hidden_dim, input_length, num_heads_for_time,
                                               num_heads_for_node, num_moe_layers, num_expert,
...
if __name__ == '__main__':
    ...
    # 测试代码
    gat = RGNN(in_dim=128, hidden_dim=256, input_length=64, num_heads_for_time=8,
                     num_heads_for_node=8, num_moe_layers=2, num_expert=4, top_k=2,
